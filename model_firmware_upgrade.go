...
// FirmwareUpgrade The Upgrade object facilitates the firmware upgrade process for rack and blade servers, providing a structured approach to downloading and applying firmware images from various sources, including cloud repositories and network shares. #### Purpose The Upgrade object is central to orchestrating firmware upgrades, offering a standardized method to enhance server functionality and security through timely firmware updates. It supports both direct download and network share upgrade methods. #### Key Concepts - **Flexible Upgrade Options:** Supports direct download from Cisco repositories and network share-based upgrades, accommodating diverse infrastructure setups and preferences. - **Policy Integration:** Seamlessly interacts with firmware policies, ensuring compliance and alignment with organizational upgrade strategies. - **Comprehensive Management:** Facilitates the entire upgrade lifecycle, from image selection and download to installation and verification, ensuring a reliable upgrade experience. - **Secure and Controlled:** Enforces strict access controls and licensing requirements to safeguard the upgrade process, maintaining system security and compliance.
type FirmwareUpgrade struct {
	...
	// Name of the server on which the firmware upgrade operation is performed.
...
func (o *FirmwareUpgrade) UnmarshalJSON(data []byte) (err error) {
	...
	if err == nil {
		...
		varFirmwareUpgrade.ServerName = varFirmwareUpgradeWithoutEmbeddedStruct.ServerName
