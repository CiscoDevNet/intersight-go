...
// TemplateTransformationStage ### Overview In Intersight workflows, task inputs may require data in a specific format. When the available value does not match the expected form, it must be transformed before it can be used. #### Purpose Transformation stages allow users to apply a sequence of data transformations. Each stage processes the output from the previous stage, ensuring that the final value matches the format required by the task input parameter in the workflow definition. #### Key Concept: - **Data Conversion:** - Each transformation stage defines how input data is converted or modified before being mapped to a task input parameter. This ensures that data conforms to the required format and structure defined in the workflow. - **Chained Processing:** - Multiple transformation stages can be linked together, where each stage processes the output of the previous one. This sequential approach ensures that the final transformed value precisely matches the format expected by the workflow task. - **Traceability:** - All transformation details are retained within the workflow and task definitions, enabling visibility, auditability, and easier troubleshooting of data flow within the workflow.
type TemplateTransformationStage struct {
	...
	Name                 *string `json:"Name,omitempty" validate:"regexp=^$|^[a-zA-Z0-9]{1,64}$"`
...
// GetDefaultClassId returns the default value "template.TransformationStage" of the ClassId field.
func (o *TemplateTransformationStage) GetDefaultClassId() interface{} {
	return "template.TransformationStage"
}
...
// GetDefaultObjectType returns the default value "template.TransformationStage" of the ObjectType field.
func (o *TemplateTransformationStage) GetDefaultObjectType() interface{} {
	return "template.TransformationStage"
}
