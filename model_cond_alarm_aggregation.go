...
// CondAlarmAggregation The AlarmAggregation object is designed to represent aggregated alarm data for a managed endpoint within Intersight. It offers a consolidated view of alarm counts and severity levels, supporting efficient monitoring and management of endpoint health. #### Purpose AlarmAggregation provides a summary of alarm data, aiding users in understanding the overall health and alarm status of managed endpoints. It is essential for assessing the impact of alarms on system performance and decision-making. #### Key Concepts - **Summary of Severity:** Aggregates alarm counts based on severity types (Critical, Warning, Info), giving users a clear view of alarm distribution. - **Endpoint Health Assessment:** Evaluates and reports the health status of endpoints based on aggregated alarm data, supporting proactive management. - **Relationship-Driven:** Tied to alarm aggregation sources within the managed object hierarchy, ensuring accurate and relevant aggregation data. - **Read-Only Access:** Designed for safe consumption, allowing users to integrate and assess alarm data without modifying it directly.
