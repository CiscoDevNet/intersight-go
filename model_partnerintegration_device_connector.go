...
// PartnerintegrationDeviceConnector The DeviceConnectors object acts as a recipe for building and deploying device connector services on an appliance. #### Purpose It manages the lifecycle of device connector images, providing a mechanism to trigger builds, deployments, and file uploads to storage buckets. #### Key Concepts - **Action Orchestration:** Supports build, deploy, and upload actions to manage service images. - **Status Monitoring:** Tracks the build status (e.g., InProgress, Completed, Failed) to ensure successful service delivery. - **Image Management:** Stores metadata regarding built docker images and source file locations.
