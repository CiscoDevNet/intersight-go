...
// StorageHitachiVolumeMigrationPair The HitachiVolumeMigrationPair object defines a copy pair used for volume migration within the Hitachi storage array, focusing on migration operations and pair management. #### Purpose HitachiVolumeMigrationPair serves as a pivotal element for managing volume migration pairs, supporting operations related to copy modes and pair statuses. #### Key Concepts - **Migration Operations:** Manages copy modes, LDEV numbers, and status information for volume migration pairs. - **Privilege Sets:** Ensures secure access and management of migration pair settings with defined privilege sets. - **Licensing:** Operates under specified entitlements, supporting authorized migration operations.
