...
// SchedulerTaskResult TaskResults represent individual execution instances produced by a TaskSchedule. For recurring schedules, each run yields a TaskResult record; for one-time schedules, there is typically a single result. TaskResults capture timing and outcome status information for auditing and troubleshooting. #### Purpose Provide per-run execution history for scheduled tasks so users and administrators can track success or failure over time and diagnose issues. #### Key Concepts - **Execution instance record:** One TaskResult per schedule run (many for recurring, one for one-time). - **Timing and duration visibility:** Captures start time, timezone, and end time for each execution. - **Outcome reporting:** Stores a structured status detail with state and reason for failure or suspension. - **Backreference to schedule:** Always linked to the TaskSchedule that produced it (cascade delete with the schedule). - **Associated object pointer:** Can reference the managed object created/updated by the run (e.g., a workflow execution record).
