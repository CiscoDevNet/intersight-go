...
// BulkResult ### Overview The Result object serves as a status-monitoring resource within asynchronous bulk operations. This provides detailed insights into the processing status of bulk API requests.    #### Purpose  Result is used to track the progress and outcome of bulk operations, offering transparency and accountability for asynchronous API transactions.   #### Key Concepts - **Status Monitoring:** - Provides real-time updates on the processing status of requests, including success, failure, and completion states.  - **Error Handling:** - Captures error details and status messages, facilitating troubleshooting and resolution of failed operations.  - **Async Support:** - Integrates seamlessly with asynchronous workflows, supporting efficient management of large-scale API operations.  - **Workflow Integration:** - Links to workflow objects for comprehensive status tracking and management within automation processes.
