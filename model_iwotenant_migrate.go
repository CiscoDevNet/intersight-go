...
// IwotenantMigrate Migrates provide an API surface to transfer a customer's IWO data, including the information required to stage migration artifacts (for example, an S3 URL) and protect access with a password that meets complexity requirements. #### Purpose Enable controlled migration of IWO tenant data for an account, coordinating the migration operation with the account and its associated tenant. #### Key Concepts - **Migration staging location:** Uses a URL (for example, an object storage bucket endpoint) to upload or access migration data. - **Protected operation:** Requires a password (stored securely/encrypted) to safeguard migration artifacts or processes. - **Account and tenant linkage:** Ties the migration request to the relevant account and (optionally) the tenant for context and governance. - **Lifecycle controls:** Supports create/update for initiating/configuring migration and delete for administrative cleanup.
