...
// OsConfigurationFile ConfigurationFiles are operating system “answer files” used to enable unattended OS installation (for example, kickstart, unattended XML, or cloud-init/seed-style files). A ConfigurationFile can be either a static file or a template containing placeholders that are resolved at install time. #### Purpose Provide reusable, centrally managed OS installation configuration content—either as fixed answers or parameterized templates—so OS installs can be automated consistently across servers. #### Key Concepts - **Unattended install enablement**: Stores OS-specific answer content used by installation workflows. - **Template support with placeholders**: `fileContent` may use Go `text/template` syntax; `placeholders` exposes detected placeholder names so UIs/workflows can collect required inputs dynamically. - **Pre-canned vs user-defined entries**: `supported` distinguishes Intersight-provided, QA-validated templates from user-created ones; `internal` can mark wizard-uploaded files that should not appear in general management views. - **OS applicability mapping**: `distributions` links the configuration file to one or more supported OS distributions (`hcl.OperatingSystem`). - **Licensed capability**: CRUD operations require the Advantage entitlement. - **Stable identity**: Identified by `name` (and `catalog` as part of identity in the model), enabling consistent referencing.
